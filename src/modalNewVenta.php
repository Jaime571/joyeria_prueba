<div class="modal fade" id="modalNewVenta" tabindex="-1" aria-labelledby="modalNewVentaLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="modalNewVentaLabel">Registrar Venta</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="saveVenta.php" method="POST" enctype="multipart/form-data">
                    <div class="mb-3">
                        <label for="id_cliente" class="form-label text-bg-info rounded p-2 mb-2 text-light">Cliente:</label>
                        <select name="id_cliente" id="id_cliente" class="form-select">
                            <option value="">Seleccionar Cleinte</option>
                            <?php foreach ($rows as $row) { ?>
                                <option value="<?= $row['id'] ?>"><?= $row['nombre'] . $row['a_paterno'] . $row['a_materno'] ?></option>
                            <?php } ?>
                        </select>
                    </div>
                    <div class="mb-3">
